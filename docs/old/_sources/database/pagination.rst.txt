		...
		<th:block fragment="bookList">
			...
			<div id="loadMoreBooks" th:unless="${books.last}">
				<a class="yadaAjax" 
					...
					yada:paginationHistory="'page, size, loadPrevious'"
...
The ``yada:paginationHistory`` attribute will add the ``page``, ``size`` and ``loadPrevious`` request
...
pages up to the current one will be fetched from database and shown. In particular, the ``loadPrevious=true`` 
parameter is the one needed to load all elements previously loaded via pagination.
