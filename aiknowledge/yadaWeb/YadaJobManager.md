...
| `disableAndInterruptJobs` | Set a list of jobs to DISABLED then interrupt the threads. |
| `completeJob` | Set a job to COMPLETED |
| `toggleDisabledAndPaused` | Toggle between paused and disabled. |
| `disableAndInterruptJob` | Set a job to DISABLED then interrupt its thread |
| `pauseAndInterruptJob` | Set a job to PAUSED then interrupt its thread |
