...
| `isPreserveImageExtension` | Check if the image extension has to be preserved when converting. |
...
| `getWebappAddress` | Return the webapp address without a trailing slash. |
| `getRoleSpringName` | Returns the Spring Security role name for a configured role ID. |
| `getRoleIds` | Convert from role names to role ids |
