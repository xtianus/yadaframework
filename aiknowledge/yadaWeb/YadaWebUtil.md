...
| `isFragment` | Check if an object holds a thymeleaf fragment instance. |
...
| `addOrUpdateUrlParameter` | Add a url parameter or change its value if present |
...
| `redirectString` | Create a redirect string to be returned by a @Controller, taking into account the locale in the path. |
