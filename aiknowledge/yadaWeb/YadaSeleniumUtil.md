...
| `getByJavascript` | Return a value calculated via javascript. |
| `typeAsHuman` | Insert some text slowly into a field |
| `findOrNull` | Return the first element matched by the selector, or null if not found |
...
| `foundById` | Check if an element with a given id exists |
| `foundByClass` | Check if at least one element with the given class exists |
| `makeWebDriver` | Create a new browser instance positioning the window |
