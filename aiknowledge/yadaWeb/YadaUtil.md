...
| `addIfNotNull` | Adds an element to the list only if the element is not null |
| `addIfMissing` | Adds an element to the list only if not there already |
| `getEmptySortedSet` | Creates a new TreeSet that sorts values according to the order specified in the parameter. |
...
| `getIsoDateStringForTimezone` | Converts a date in the timezone to a ISO string, like '2011-12-03' |
| `getIsoTimeStringForTimezone` | Converts a time in the timezone to a ISO string, like '10:15' or '10:15:30' |
| `getIsoDateTimeStringForTimezone` | Converts a datetime in the timezone to a ISO string, like '2011-12-03T10:15:30' |
| `makeJsonObject` | Creates a single empty "json" object for use in other methods. |
...
| `mapToString` | Converts a map of strings to a commaspace-separated string of name=value pairs |
...
| `splitHtml` | Splits an HTML string in two parts, not breaking words, handling closing and reopening of html tags. |
| `findAvailableFilename` | Ensure that the given filename has not been already used, by adding a counter. |
| `dateValid` | Checks if a date is not more than maxYears years from now, not in an accurate way. |
...
| `formatTimeInterval` | Converts from an amount of time to a string in the format xxd:hh:mm:ss |
...
| `stripCounterFromFilename` | Removes a counter that has been added by #findAvailableName |
...
| `md5Hash` | Creates a MD5 hash of a string (from http://snippets.dzone.com/posts/show/3686) |
...
| `isCollection` | Checks if the last segment of an attribute path is a Collection (List, Set, etc.) |
| `getMessage` | Returns a message. |
| `getNewInstanceSamePackage` | Creates an instance of a class that belongs to the same package of some given class |
| `getClassesInPackage` | Returns all the classes of a given package. |
...
| `deleteAll` | Deletes all files in a folder that have the specified prefix |
| `deleteSilently` | Deletes a file ignoring errors. |
...
| `isType` | Checks if a class is of a given type, considering superclasses and interfaces (of superclasses) |
| `dateWithin` | Checks if a date is within two dates expressed as month/day, regardless of the year and of the validity of such dates. |
...
| `roundBackToMidnightClone` | Creates a new calendar rounded back to the start of the day. |
...
| `createZipProcess` | Creates a zip of a set of files using an external process. |
| `createZipFileFromFolders` | Creates a zip file of a folder |
| `createZipFile` | Creates a zip of a list of files. |
