...
It can also be specified as an HTML attribute: `yada:ajaxElementLoader`
...
When Spring Security CSRF protection is enabled, the page must expose the CSRF token in standard meta tags so that `yada.ajax.js` can add the CSRF header to same-origin ajax requests that use unsafe HTTP methods such as POST, PUT, PATCH, or DELETE.

[source,html]
----
<!--/* CSRF token exposed to JS so yada can attach it to AJAX requests */-->
...
These tags belong in the HTML page `<head>`, usually in the shared header fragment. See `YadaExamples/src/main/webapp/WEB-INF/views/header.html` for the reference placement.
