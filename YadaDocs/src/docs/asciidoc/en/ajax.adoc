...
yada:successHandler="alert(responseText), alert('hello')"
----
...
[WARNING] 
==== 
When using both `yada:updateOnSuccess` and `yada:successHandler`, the *responseHtml* argument
is whatever was returned by the ajax call and could have multiple root objects. 
When `yada:successHandler` is used alone though, the *responseHtml* is added to a parent `<div>`.
====
