...
You can customize how cell values are displayed by providing a javascript render function.
This is useful for wrapping values in HTML tags, applying conditional formatting, or transforming data before display.

Use the `.dtRender()` method on a column to specify a javascript function that will be called for each cell:

[source,java]
----
YadaDataTable yadaDataTable = yadaDataTableFactory.getSingleton("myTable", locale, table -> {
	table
		.dtEntityClass(MyEntity.class)
		.dtStructureObj()
			.dtColumnObj("Name", "name")
				.dtRender("renderNameCell") # <1>
				.back()
		.back();
});
----
<1> Specify the name of a javascript function to render the cell

On the HTML page, define the render function. The function signature follows the DataTables convention:

[source,javascript]
----
function renderNameCell(data, type, row, meta) {
	return '<span class="myClass">' + data + '</span>';
}
----

The function receives:
- `data`: the cell value from the data source
- `type`: the type of data requested (filter, display, sort, type)
- `row`: the full data object for the row
- `meta`: an object with metadata about the cell (row index, column index, etc.)

Return the HTML string to be displayed in the cell. This allows you to wrap values in tags, add icons, apply conditional styling, or create complex cell content based on the row data.

=== Column Sort Sequence

By default, DataTables cycles through three sorting states when a column header is clicked: ascending, descending, and no sorting.
This means that clicking a column header three times will remove the sort entirely.

You can customize this cycling behavior using `dtOrderSequence()` to control which directions are available.
For example, you can prevent users from removing the sort by excluding the empty string from the sequence.

==== Table-wide configuration
Set the sort sequence for all columns at once using `dtOptionsObj().dtOrderSequence()`:
...
YadaDataTable yadaDataTable = yadaDataTableFactory.getSingleton("myTable", locale, table -> {
	table
		.dtEntityClass(MyEntity.class)
		.dtStructureObj()
			.dtColumnObj("Name", "name").back()
			.dtColumnObj("Date", "createdDate").dtOrderDesc().back()
		.back()
		.dtOptionsObj()
			.dtOrderSequence("asc", "desc") // <1>
			.back()
		;
});
----
<1> All columns will cycle between ascending and descending only, never removing the sort

NOTE: The table-level `dtOrderSequence()` sets the same sequence for all columns that don't have a per-column setting. It is a Yada extension and has no direct equivalent in the official DataTables API. If you need different sequences for different columns, use the per-column `dtOrderSequence()` method.

==== Per-column configuration
Override the sort sequence on individual columns using `dtOrderSequence()` on the column:
...
.dtStructureObj()
	.dtColumnObj("Name", "name")
		.dtOrderSequence("asc", "desc") // <1>
		.back()
	.dtColumnObj("Priority", "priority")
		.dtOrderSequence("desc", "asc") // <2>
		.back()
.back()
----
<1> This column cycles between ascending and descending only
<2> This column starts with descending and never allows "no sort"

NOTE: The per-column `dtOrderSequence()` overrides any table-wide setting from `dtOptionsObj().dtOrderSequence()`.

Valid values for the sequence are `"asc"`, `"desc"`, and `""` (empty string for no sorting).
...
=== Reload after row change
As shown in the previous section, the table can be reloaded with fresh data using the `reload()` API method.
A common case is to reload the table after the row has been changed in an edit modal. For example, the
user table may have an "edit" button that opens a modal with a form to edit the user: after submitting the
new values, the table contents should be updated to reflect the changes. This is achieved using standard Yada Framework logic
by calling the reload method in the form success handler:

[source,java]
----
<form name="userForm" yada:successhandler="$('#userTable').data('yadaDataTableApi').ajax.reload(null, false)" # <1>
	th:action="@{/saveUser}" th:object="${userProfile}" class="yadaAjax">
----
<1> the success handler is invoked after the form has been submitted and the ajax call has returned, therefore any changes to the data have been executed and will be shown after reload

=== Custom Text Search

You can add a simple custom text search input to filter the data shown in your DataTable across all searchable columns, in case the default search box one is not suitable for your needs.
The DataTables built-in global search is handled automatically by `YadaDataTableDao`: when the search value is set, it generates LIKE queries on all columns that have `searchable` enabled, with no additional backend code required.

On the HTML page, add a text input anywhere and use the postprocessor to wire it to the DataTables `search()` API with a debounce:

[source,html]
----
<input type="text" class="mySearchInput" />

<yada:datatable
	yada:configuration="${myTableAttribute}"
	yada:postprocessor="myTablePostprocessor">
</yada:datatable>
----

[source,javascript]
----
function myTablePostprocessor(dataTableApi, dataTableJson) {
	var timer;
...
The `search(value).draw()` call sends the value as the standard DataTables `search[value]` parameter.
On the backend, `YadaDataTableDao.getPage()` reads it automatically and applies a LIKE search on all searchable columns of the entity. No changes to the ajax endpoint are needed.

=== Multi-field Filter

You can add structured form elements (text inputs, checkboxes, selects) to filter the data shown in your DataTable with custom server-side logic.
