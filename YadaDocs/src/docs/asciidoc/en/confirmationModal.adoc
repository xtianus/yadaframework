...
The `message` and `callback` parameters are the text to show and the function to call on user choice (confirmation or eventually cancellation).
...
If the `callback` is defined without parameters, it will only be called on positive confirmation. If it is defined with a parameter,
it will always be called and will receive `true` for confirmation and `false` for abort.

[source,javascript]
----
yada.confirm("Confirm deletion", "Are you sure you want to delete this?", function(result) {
	if (result) { 
		doSomeDeletion();
	} else {
		doSomthingElse();
