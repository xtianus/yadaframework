...
===  Scattered Forms (form groups)

Multiple forms scattered around the page can be submitted together if they belong to the same "Form Group":
when any ajax form in the group is submitted, the fields from all other forms in the same group are added
to the payload.
When a non-ajax form is submitted, the group is ignored.
...
[source,html]
----
<form action="/realAction" yada:formGroup="myGroup" class="yadaAjax">
...
In the above example, submitting the first form would send `"a=1&b=2&c=3"` to `/realAction`.
...
- does not prevent the individual submission of non-ajax forms in the group the standard way
- can be used together with ajax calls on elements other than forms (e.g. <<../ajax.adoc#_ajax_links,ajax links>>)  
