...
create table YadaUserProfile (timezoneSetByUser bit not null, avatar_id bigint, id bigint not null auto_increment, userCredentials_id bigint not null, version bigint not null, DTYPE varchar(31) not null check ((DTYPE in ('YadaUserProfile','UserProfile'))), firstName varchar(32), locale varchar(32), middleName varchar(32), nickname varchar(32), lastName varchar(64), timezone varchar(64), primary key (id)) engine=InnoDB;
create table YexRegistrationRequest (id bigint not null, name varchar(255), surname varchar(255), primary key (id)) engine=InnoDB;
alter table YadaArticle add constraint UK7vechuxf3aovt7qyl8367akf6 unique (image_id);
alter table YadaArticle add constraint UKnhcjr5g5me98n5drrkfg1p7rc unique (sku);
alter table YadaArticle_attachments add constraint UKckp9t4579t7mov1miiu8fql4f unique (attachments_id);
alter table YadaArticle_galleryImages add constraint UKrl1gkdyfrt43m5jw3u598olgk unique (galleryImages_id);
alter table YadaArticle_silhouetteImages add constraint UKsq5igv7krtxe04nyyhlx4djeg unique (silhouetteImages_id);
...
alter table YadaOrder add constraint UK8iybl8cd5lhj034f5gahfoymk unique (orderStatus_id);
alter table YadaPersistentEnum add constraint UKfuc71vofqasw0r57t7etipp7p unique (enumClassName, enumOrdinal);
alter table YadaProduct add constraint UKaaflxyksnnr4ut62k1khkw4co unique (image_id);
alter table YadaProduct_attachments add constraint UKfsu5mkfbdh7dvoq619df26n50 unique (attachments_id);
alter table YadaProduct_categories add constraint UK1cmx2j13ct60yhcy0ef59lny1 unique (YadaProduct_id, categories_id);
alter table YadaProduct_galleryImages add constraint UKpaqmx4kvpgkaxj40cxlbxnv9h unique (galleryImages_id);
...
alter table YadaTicket add constraint UKbbcda6270hf9977unmh0rt3ua unique (status_id);
alter table YadaTicket add constraint UKcp5v1tqelobdtcaoeus3lr2r7 unique (type_id);
alter table YadaTransaction add constraint UKmt2464fv1orqfodyjr5pa8vcv unique (order_id);
alter table YadaUserCredentials add constraint UK6gbgs7fb7g5t4wo0ys7e5q31j unique (username);
alter table YadaUserMessage add constraint UKrebeqry21dwm7f3q3yi5xuro1 unique (type_id);
alter table YadaUserMessage_YadaAttachedFile add constraint UK9jwh2bf420o62f2r5aa5eapo9 unique (attachment_id);
